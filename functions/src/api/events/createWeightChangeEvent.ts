...
/**
 * POST /api/events
 * Creates a new "weightChange" event.
 * Expects JSON body with:
 *   - Required: userId, dogId
 *   - Optional: eventDate, weight
 * 
 * Dog snapshot data is fetched from the dog document.
 */
export const createWeightChangeEvent = functions.https.onRequest({ cors: true }, async (req, res) => {
  try {
    const { userId, dogId, eventDate, weight, imageUrls } = req.body;
    if (!userId || !dogId) {
    ...
    console.error("Error creating weight change event:", error);
