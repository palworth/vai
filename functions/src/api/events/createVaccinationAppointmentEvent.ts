...
/**
 * POST /api/events
 * Creates a new "vaccinationAppointment" event.
 * Expects JSON body with:
 *   - Required: userId, dogId
 *   - Optional: eventDate, vaccinationsType, vetName, notes, vetDocuments (array)
 */
export const createVaccinationAppointmentEvent = functions.https.onRequest({ cors: true }, async (req, res) => {
  try {
    const { userId, dogId, eventDate, vaccinationsType, vetName, notes, imageUrls } = req.body;
    if (!userId || !dogId) {
    ...
    console.error("Error creating vaccination appointment event:", error);
