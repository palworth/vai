...
def scrape_dog_breed(driver, url):
    """Scrapes data from a given dog breed URL and returns a dictionary with scraped information."""
    driver.get(url)
    time.sleep(2)
...
def main():
    # Read the JSON file with the list of dog breeds and their URLs.
    with open("akc_dog_breeds.json", "r", encoding="utf-8") as f:
        breeds = json.load(f)
    ...
    # Loop over each breed in the JSON file and scrape its respective page.
    for breed in breeds:
        name = breed.get("name")
        url = breed.get("url")
        print(f"Scraping data for {name} from {url}")
        try:
            breed_data = scrape_dog_breed(driver, url)
            results[name] = {
                "url": url,
                "scraped_data": breed_data
            }
        except Exception as e:
    ...
    with open("scraped_dog_breeds.json", "w", encoding="utf-8") as f:
        json.dump(results, f, indent=4)
    
    print("Scraping complete. Data saved to scraped_dog_breeds.json")
